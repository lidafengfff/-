...
function checkName(e) {
    ...
    $.ajax({
		...
		success:function (data) {
			if (!data){
                $('#regist_username').attr('data-content','用户名以存在');
                $('#regist_username').popover('show');
			}
        }
	})
...
function login() {
    ...
    if (password==null || password.length==0){
        $('#password').attr('data-content','用户名不能为空');
...
function changepwd(){
    ...
    if (lastPassword==null||lastPassword.length==0){
        $('#last_password').attr('data-content','媛密码不能为空');
        $('#last_password').popover('show');
        return;
    }
    if (newPassword == null||newPassword.length <6){
        $('#new_password').attr('data-content','新密码不能小于6位');
